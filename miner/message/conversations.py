...
class Conversations:
    ...
    def __init__(self, path: str) -> None:
        ...
        self._private: Dict[str, Conversation] = self.get_convos(directories=paths_factory.get_dirs(ctype='private'),
                                                                 dir_lister=self.get_json_paths)
        self._group: Dict[str, Conversation] = self.get_convos(directories=paths_factory.get_dirs(ctype='group'),
                                                               dir_lister=self.get_json_paths)
    ...
    def get_convos(self, directories: List[str], dir_lister: Callable) -> Dict[str, Conversation]:
        name_convo_map = {}
    ...
    @staticmethod
    def merge_convo_files_if_needed(convo_map: Dict[str, Conversation], jsons: List[str]) -> Dict[str, Conversation]:
        try:
...
class ConversationsPaths:
    ...
    def get_convos_paths(self) -> None:
        ...
        for directory in directories:
            convo = Conversation(path=f'{directory}/message_1.json', processors=[])  # NOTE we don't want to process it
