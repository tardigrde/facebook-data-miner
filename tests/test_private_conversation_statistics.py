...
def priv_stats(panalyzer):
    def _stats(**kwargs):
        if "participants" in kwargs or "channels" in kwargs:
            analyzer = panalyzer.filter(
                channels=kwargs.get("channels"), participants=kwargs.get("participants")
            )
        else:
            analyzer = panalyzer
        if any([kw in kwargs for kw in ("senders", "start", "end")]):
...
class TestPrivateStatisticsWithFiltering:
    ...
    def test_stats_teflon_musk(self, panalyzer):
        stats = panalyzer.filter(channels="Teflon Musk").stats
        assert stats.mc == 6
        assert stats.wc == 14
        assert stats.cc == 52
    ...
    def test_stats_teflon_musk_me(self, panalyzer):
        stats = panalyzer.filter(channels="Teflon Musk",).stats.filter(senders="me")
        assert stats.mc == 3
        assert stats.wc == 12
        assert stats.cc == 48
        assert stats.unique_mc == 1
    ...
    def test_stats_teflon_musk_partner(self, panalyzer):
        stats = panalyzer.filter(channels="Teflon Musk").stats.filter(senders="partner")
        assert stats.mc == 3
        assert stats.wc == 2
        assert stats.cc == 4
        assert stats.unique_mc == 1
    ...
    def test_stats_teflon_musk_all_2014_9(self, panalyzer):
        stats = panalyzer.filter(channels="Teflon Musk").stats.filter(
